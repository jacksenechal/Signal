// GUI components generated by Fast Light User Interface Designer (fluid) version 1.0110
...
void PortMidiTest_midiThruCleanup(void* arg) {
    cout << "midi thread cleanup...\n";
}

void* PortMidiTest_midiThru(void* arg) {
    pthread_cleanup_push(PortMidiTest_midiThruCleanup, arg);
    PortMidiTest* pmtest = static_cast<PortMidiTest*>(arg);
    while (true) {
        pthread_mutex_lock(&(pmtest->midiThruButtonMutex));
        pthread_cond_wait(&(pmtest->midiThruButtonCondition), &(pmtest->midiThruButtonMutex));
        printf("Midi through button is %s\n",  pmtest->buttonState ? "ON" : "OFF" );
        pthread_mutex_unlock(&(pmtest->midiThruButtonMutex));
    }
...
PortMidiTest::PortMidiTest() {
    /* Initialize mutex and condition variable objects */
    pthread_mutex_init(&midiThruButtonMutex, NULL);
    pthread_cond_init (&midiThruButtonCondition, NULL);

    /* Create the thread */
    int rc = pthread_create(&thread, NULL, PortMidiTest_midiThru, this);
    if (rc){
        printf("ERROR; return code from pthread_create() is %d\n", rc);
        exit(-1);
    }
    cout << "created midi thread\n";
}

PortMidiTest::~PortMidiTest() {
    /* Destroy mutex and condition variable objects */
    pthread_mutex_destroy(&midiThruButtonMutex);
    pthread_cond_destroy(&midiThruButtonCondition);
    cout << "cancelling midi thread\n";
    pthread_cancel(thread);
}

/* Start the thread for the PortMidi test */
void PortMidiTest::start() {
    pthread_mutex_lock(&midiThruButtonMutex);
    buttonState = 1;
    pthread_cond_broadcast(&midiThruButtonCondition);
    pthread_mutex_unlock(&midiThruButtonMutex);
}
